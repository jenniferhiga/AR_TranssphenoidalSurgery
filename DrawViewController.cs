...
public class DrawViewController : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler {
  ...
  [SerializeField] private string imageName;
    ...
    void Update() {
        ...
        if(clearCanvas)
        {
          SaveTexture2DToFile(drawTexture, imageName, SaveTextureFileFormat.PNG, 95);
          ResetTexture();
          clearCanvas = false;
       }
     }
...
#if UNITY_EDITOR
        bool isShiftHeldDown = Keyboard.current.leftShiftKey.isPressed|| Keyboard.current.rightShiftKey.isPressed;
        bool isZHeldDown = Keyboard.current.zKey.wasPressedThisFrame;
        bool isYHeldDown = Keyboard.current.yKey.wasPressedThisFrame;
...
#else
      bool isControlHeldDown = Keyboard.current.leftCtrlKey.isPressed || Keyboard.current.rightCtrlKey.isPressed;
      bool isZHeldDown = Keyboard.current.zKey.wasPressedThisFrame;
